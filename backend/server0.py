...
async def personnel_consent_tracking(
    ...
    if q:
        qq = q.strip().lower()
        rows = [r for r in rows if qq in ((r.get("ad_soyad") or "").lower())
                or qq in ((r.get("sicil_no") or "").lower())]
    if departman:
...
async def consent_tracking(
    ...
    if q:
        ...
        items = [it for it in items if
                 qq in ((it.get("ad_soyad") or "").lower()) or
                 qq in ((it.get("sicil_no") or "").lower())]
    if departman:
...
def _build_personnel_filter(q: Optional[str], departman: Optional[str],
    ...
    if q:
        # regex escape kullanıcı input'u
        esc = re.escape(q.strip())
        filt["$or"] = [
            {"ad_soyad": {"$regex": esc, "$options": "i"}},
            {"sicil_no": {"$regex": esc, "$options": "i"}},
            {"tc_no": {"$regex": esc, "$options": "i"}},
        ]
    return filt
